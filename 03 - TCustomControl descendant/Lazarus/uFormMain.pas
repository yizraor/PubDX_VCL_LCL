...
uses
  Windows, Messages, LCLType, SysUtils, Variants,
...
begin
  ...
  hr := D3D11CreateDevice
        (
          ...
          IfThen(TFormMain(Owner).actOtherDebugDevice.Checked, D3D11_CREATE_DEVICE_DEBUG, 0),
  ...
  if ( TFormMain(Owner).actOtherDebugDevice.Checked ) then
  begin
...
begin
  ...
  if ( TFormMain(Owner).actRenderVSync.Checked ) then
  begin
...
begin
  pRenderPanel := TRenderPanel.Create(Self);
  pRenderPanel.Align := alClient;
  pRenderPanel.Parent := Self;
  ...
  try
    ...
    pRenderPanel.InitShaders(sShaderCode_Sample_1, pRenderPanel.pCompiledVertexShader, pRenderPanel.pCompiledPixelShader);
    pRenderPanel.InitDirect3D();
  except
...
begin
  ...
  if ( pRenderPanel <> nil ) and
     ( pRenderPanel.bInitComplete ) then
    InvalidateRect(pRenderPanel.Handle, nil, FALSE);
end;
...
begin
  ...
  pRenderPanel.InitShaders(sCode, pVS, pPS);

  if ( pVS <> nil ) and ( pPS <> nil ) then
  begin
    pRenderPanel.FinalizeDirect3D();

    pRenderPanel.pCompiledVertexShader := pVS;
    pRenderPanel.pCompiledPixelShader := pPS;
...
begin
  ...
  if ( pRenderPanel.pD3Ddebug = nil ) then
  begin
    Application.MessageBox('Не найден отладочный интерфейс ID3D11Debug!', 'ошибка', MB_OK or MB_ICONERROR);
    Exit;
  end;

  pRenderPanel.pD3Ddebug.ReportLiveDeviceObjects(D3D11_RLDO_DETAIL);
end;
